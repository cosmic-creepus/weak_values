...
def nroot_swap_weak_value_vectors(i, f, q: int, n: int,
  ...
  """
    ...
    for nr in range(0, l):
        ...
        first_derivative_sa, second_derivative_sa = compute_derivatives(tau=1 / (2 ** n) * nr, alpha=1 / (2 ** n) * (iterations - 1), psi_f=f, psi_i=i, spinors=SpinOps(q=q))
        first_derivative_sb, second_derivative_sb = compute_derivatives(tau=1 / (2 ** n) * nr, alpha=1 / (2 ** n) * (iterations - 1), psi_f=f, psi_i=i, spinors=SpinOpsR(q=q))
...
num_state = 0  # number of states to iterate
success = []
a_val = []
csv_data = []
run = 0
